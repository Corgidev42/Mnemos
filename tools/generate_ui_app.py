#!/usr/bin/env python3
"""Génère mnemos/ui/app.py à partir du corps de QuizApp dans quiz_rappel_gui.py."""
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
MONO = ROOT / "quiz_rappel_gui.py"
OUT = ROOT / "mnemos" / "ui" / "app.py"
...
def main():
    ...
    if "class QuizApp(tk.Tk):" not in mono_text:
        print(
            "Skip: quiz_rappel_gui.py est un shim sans QuizApp. "
            "Modifier mnemos/ui/screens/*.py ou régénérer depuis un monolithe Git.",
        )
        return
    lines = mono_text.splitlines(keepends=True)
    # class QuizApp starts line 1272 (1-based) -> index 1271
    # last line of class before if __name__ is 4332 (1-based) -> index 4331
    # class QuizApp ligne 1272 (1-based) ; fin de classe avant « Point d'entrée »
    body = "".join(lines[1271:4326])
    OUT.write_text(HEADER + "\n" + body + FOOTER, encoding="utf-8")
    print("Wrote", OUT, "lines", len((HEADER + body + FOOTER).splitlines()))
